...
const userFeedback = {};
const userName = {};
const userInteractions = {};  // To track user interactions
...
app.post('/webhook', async (req, res) => {
    ...
    if (!userName[from]) {
        if (userState[from] !== 'askingForName') {
            userState[from] = 'askingForName';
            responseMessage = "Hi there! What's your name?";
        } else {
            userName[from] = req.body.Body;
            userState[from] = 'initial';
            responseMessage = `Thanks again for sharing all that information about you, ${userName[from]}. It sounds like you had a difficult year so before caring for others, we’d like you first to focus on yourself. Kindness to oneself is just as important. Please take some time for you today with this small act: [add low effort self-care act]. Once you do, please let us know by texting “done”. Take care of yourself today!`;

            // Set reminders only if the user does not interact after this message
            userInteractions[from] = Date.now();
            setReminder(from, 6 * 1000, `Hey ${userName[from]}, don’t forget to care for yourself today with our suggested act. Or with anything else that might bring you a smile. Just take a moment today to care for you. It’s as important as caring for others. ❤️`, 'waitingForOptionAfter6Hours');
            setReminder(from, 9 * 1000, `Hi ${userName[from]}, just checking if you took a moment to do something nice for you. Please respond “Done” or “Not Yet”. Thank you for being part of this journey!❤️`, 'waitingForOptionAfter9Hours');
        }
    } else {
        userInteractions[from] = Date.now();  // Update interaction time

        if (userState[from] === 'initial') {
            if (body === 'done') {
                responseMessage = await ifActDone(from);
            } else if (body === 'not yet') {
                responseMessage = "Sometimes life gets busy, or certain acts might not resonate as much. Could you please share what prevented you from completing it until now? Your feedback will help us improve your experience.\n\n" + await sendOptions(from);
...
const setReminder = (to, delay, message, nextState = null) => {
    const timeoutId = setTimeout(async () => {
        // Check if the user has interacted within the delay period
        if (Date.now() - userInteractions[to] >= delay) {
...
const handleFeedback = async (from, body) => {
    userFeedback[from] = { feedback: body };
    userState[from] = 'waitingForCompletion';
    return "Thanks for your valuable feedback! There are many reasons why an act might not work out. We’ll take this into account for future suggestions. Remember, when we make a suggestion, you can always ask for an alternative act.";
};

const ifActDone = async (to) => {
    userState[to] = 'postActMeasures1';
    clearReminders(to);
    await sendMessage(to, `So happy to hear you took time to care for yourself, ${userName[to]}. We hope it made you feel better. Keep it up! 🌟`);
